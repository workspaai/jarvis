"""Stage 3: add retry guardrails plus model, latency, and cost fields.
...
app = FastAPI(title="Stage 3: Guardrails and Observability")
client = OpenAI()
...
def call_malformed_json_once(question: str, model: ModelName) -> tuple[str, int, int, int]:
    ...
    if usage is None:
        return raw, 0, 0, 0
    return raw, usage.total_tokens, usage.prompt_tokens, usage.completion_tokens
    ...
    for attempt in range(2):
        try:
            if body.force_bad and attempt == 0:
                ...
                answer, tokens_used, prompt_tokens, completion_tokens = call_structured(
                    body.question, model
                )
                total_tokens_used += tokens_used
                total_prompt_tokens += prompt_tokens
